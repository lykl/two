<?php
    ...
    $db=mysql_connect("localhost","root","123456");//连接数据库   (地址 用户名 密码)
    mysql_select_db("data",$db);//选择数据库 数据库名称 连接
    mysql_query("set names utf8");//设置编码格式


    $uname = $_REQUEST["myname"];
    $upassward = $_REQUEST["mypassward"];
    $usex = $_REQUEST["mysex"];
    ...
    $data = mysql_query($sql2);
        ...
        if($result){
            ...
            $sql3="INSERT INTO `my_data`(`my_id`, `my_name`, `my_passward`, `my_sex`, `my_phone`) VALUES (NULL, '$uname','$upassward','$usex','$uphone')";
